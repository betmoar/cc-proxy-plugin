...
- **`/cc-proxy:models` command.** Lists every model reachable through the proxy with the provider each one routes to, in a flat column. Reads the proxy's `GET /v1/models` and attributes ids against the registered providers' `match()` predicates (restated in `scripts/list-models.js`, locked by `test/list-models.test.js`); a failed live-fetch leg is flagged from `_errors`. Raw JSON remains one `curl http://127.0.0.1:4000/v1/models` away.
