...
### Added
- **`GET /_ping` liveness probe.** A minimal `200` with an empty body — the fastest possible "is the proxy process up" check (no JSON serialization, no config read). For richer info use `GET /_status`; `/_ping` is the hot-path up/down check (`curl -sf -o /dev/null …/_ping`). Loopback-bound, no auth — same posture as `/_status`.
