...
| `CONTEXT_WINDOW` (`models.js`)                              | every entry becomes a `modelPicker` row in the USER'S settings.json (#62), applied on their next `/cc-proxy:setup` — so adding a model edits a file OUTSIDE this repo, and the version-stamped SessionStart notice is the only thing that tells them to re-run it. `couplings.test.js` pins the count as a tripwire; bump it deliberately and re-read OPERATIONS' picker section                                                                                                                                                                                                                                                                                                                                                        |
| a writer of a file under `~/.claude`                        | stage a sibling `.tmp-<pid>` and `renameSync` over the target — `writeFileSync` opens with `'w'`, so a kill mid-write leaves the user's file truncated. `couplings.test.js` denies the spelling (tracing the variable's DECLARATION: `const tmp = file` keeps the name and restores the bug), `model-picker.test.js` pins the target's INODE, which is the only observable separating rename from truncate — "no `.tmp` survives" passes against both (measured 0.10.0) |
| `buildRows()` (`model-picker.js`)                           | `[1m]` is the ONLY window channel that survives `behavesAs` — the latter makes the id "known", which is what disables `CLAUDE_CODE_MAX_CONTEXT_TOKENS` (measured: bare id + pin + `behavesAs` = 200k, not 1m). Drop either half and nine models silently lose 800K of window, or a catalog warning returns every session. Both locked in `model-picker.test.js`                                                                                                                                                                                                                                                                                                                                    |
...
- **CC internals may drift**: `[1m]` suffix, `claude-haiku-*` ids,
  `ANTHROPIC_CUSTOM_MODEL_OPTION` (one slot), and — added 0.10.0 — the
  `modelPicker` row schema, `behavesAs`, and the gate that makes
  `CLAUDE_CODE_MAX_CONTEXT_TOKENS` apply only to ids CC calls unknown. None are
  public API — check these first when routing OR a context window looks wrong
  after a CC update. The picker three are the newest and least settled: they
  rest on one measurement (CC 2.1.263, 2026-09-07, in `model-picker.js`'s
  header) and, unlike a vendor claim, `probe:vendors` cannot re-measure them —
  it is built for keys and quota, and this probe needs the local `claude`
  binary. Re-run it BY HAND from the recipe in that header when the picker
  misbehaves; a stale window is silent, which is the whole reason #62 existed.
