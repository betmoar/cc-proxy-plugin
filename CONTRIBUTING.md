...
`models:picker` writes YOUR `~/.claude/settings.json` — it is a user-facing action, not a build step, and nothing in the repo changes. Use `--dry-run` to see the merged file or `--print` for just the rows. It exists because Claude Code assumes a 200K context window for every id its catalog does not describe, which is all of them; see docs/OPERATIONS.md "The /model picker".
