...
`preversion`/`version` are npm lifecycle plumbing: `version-guard.js` refuses a
tagging `pnpm version` on a non-`main` branch (the tag would land on a commit
the squash-merge discards — issue #41), and the repo `.npmrc` (`git-tag-version=false`)
blocks npm's tag. Bump with `pnpm version patch|minor --no-git-tag-version` on
the branch; create the tag on `main` after the squash.
