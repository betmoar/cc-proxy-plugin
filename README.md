...
- **API errors after setup** — setup starts the proxy itself, so this is usually an *already-open* session that retargeted before the proxy came up. `/exit` + `/resume` it (the SessionStart hook ensures the proxy is running). If a new session also errors, check `~/.claude/cc-proxy/cc-proxy.log` — and if the proxy failed to start, the session itself now says so: the SessionStart hook injects one line of context ("cc-proxy: the proxy did not start…") so the model can tell you before you hit the first `ECONNREFUSED` (issue #55).
