---
name: models
description: List every model reachable through cc-proxy with the provider each routes to.
argument-hint: (no arguments)
allowed-tools: Bash
disable-model-invocation: true
---

Run the cc-proxy model list and show the user its output.
...
while IFS= read -r c; do
  [ -n "$c" ] || continue
  if [ -f "$c/scripts/list-models.js" ]; then root="$c"; break; fi
done <<EOF
${CLAUDE_PLUGIN_ROOT:-}
