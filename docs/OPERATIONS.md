...
- **`ANTHROPIC_CUSTOM_MODEL_OPTION`** — exactly one slot; the id passes verbatim into `model` with validation skipped. There is no `_2`: `modelPicker` (below) is the multi-row mechanism, and `/cc-proxy:setup` removes this key when it writes picker rows: CC dedupes the picker by model id and **this env wins**, so leaving it replaces the generated row for that model with a bare one — no `behavesAs`, so the catalog warning returns for that id (measured — see the picker section below).
...
**Generate, never hand-write.** `pnpm models:picker` (`scripts/render-model-picker.js`) builds one row per curated id whose provider is registered, suffixes those ≥1M, and merges into `~/.claude/settings.json` preserving every foreign row in place — the no-merging rule above means a naive write silently discards a user's own rows. `--dry-run` prints the merged file; `--print` prints just the rows (and reads no settings at all, so it still works when settings.json is the broken thing). A `.bak` is written before any overwrite, and the overwrite itself is **atomic** — a sibling temp file renamed over the target, so a kill mid-write can never leave settings.json truncated. It refuses (exit 1, nothing written) when no provider key is registered, or when settings.json exists and does not parse. The rows are a **snapshot**: when `CONTEXT_WINDOW` changes, the SessionStart hook emits a one-line staleness notice (once per plugin version, stamped in `~/.claude/cc-proxy/picker-stamp.json`) telling the user to re-run it.
...
Since 0.9.2 the probe also prints a **catalog drift** pass (issue #37), in both plain and `--json` runs: it compares `ROUTES` and the vendor's own `/models` lists and prints `DRIFT` (an id answers 200 but serves a different model — aliasing; or an expected-200 route no longer answers 200), `STALE` (a `ROUTES` status-200 id the vendor's own list cannot see — the `/api/anthropic/v1/models`-omits-`glm-5.3` trap), and `INFO` (a shape-routed id the vendor lists but `ROUTES` does not rank — media/audio namespaces excluded). Drift lines count as disagreements for the exit code (exit 1). It never edits anything: the tables stay hand-owned (backlog item 12) and the human decides every row. First live run 2026-09-04 found: Z.ai still aliases `glm-5.2`/`glm-5.1` onto `glm-5.3` (the `ROUTES` comment's claim, re-confirmed), and the Qwen plan list omits `qwen3.8-max-preview` while `ROUTES` records it 200 — a candidate for re-probing before the next release that touches Qwen.
