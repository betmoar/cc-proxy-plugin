...
async function checkProxyAlive(port, cacheDir) {
  ...
  try {
    ...
    if (
      cached.port === port &&
      Date.now() - cached._ts < PROXY_PROBE_CACHE_TTL_MS
    ) {
      return cached.alive;
    }
  } catch {
  ...
  try {
    ...
    fs.writeFileSync(
      cachePath,
      JSON.stringify({ port, alive, _ts: Date.now() }),
    );
  } catch {
    // non-fatal
  }
...
async function loadGlmQuota(cacheDir) {
  ...
  const cachePath = cacheDir
    ? path.join(cacheDir, "glm_quota_cache.json")
    : null;
...
async function loadOpenRouterCredits(cacheDir) {
  ...
  const cachePath = cacheDir
    ? path.join(cacheDir, "openrouter_credits_cache.json")
    : null;
  if (cachePath) {
    try {
...
process.stdin.on("end", async () => {
  ...
  if (rl?.five_hour) {
    parts.push(
      renderQuota(
        "cc",
        Number(rl.five_hour.used_percentage),
