...
describe("settings merge (issue #62)", () => {
	...
	// Measured 2026-09-25 on a real settings.json: `deepseek-v4-flash[1m]` was
	// generated by 0.10.1, then de-curated in 0.10.2 when DeepSeek retired the
	// name. Keyed on CONTEXT_WINDOW alone, the row turned "foreign" and every
	// later setup preserved it — issue #30's stale row, reached by de-curation
	// instead of a removed key. The row's own description is our signature.
	it("recognizes a row it generated for an id that is no longer curated", () => {
		const stale = {
			model: "deepseek-v4-flash[1m]",
			label: "DeepSeek V4 Flash (1M)",
			description: "1M context, routed via cc-proxy",
			behavesAs: "claude-sonnet-5",
		};
		assert.equal(isGeneratedRow(stale), true);
		assert.equal(
			isGeneratedRow({ ...stale, description: "128K context, routed via cc-proxy" }),
			true,
		);
		// A user's row for the same id with their own words stays theirs.
		assert.equal(isGeneratedRow({ ...stale, description: "my notes" }), false);
		assert.equal(
			isGeneratedRow({ model: "vendor/x", description: "routed via cc-proxy, sort of" }),
			false,
		);
		// The signature must cover every description buildRows() can emit, or a
		// window added later leaves its rows unrecognizable once de-curated.
		for (const w of new Set(Object.values(CONTEXT_WINDOW))) {
			const description = `${formatWindow(w)} context, routed via cc-proxy`;
			assert.equal(isGeneratedRow({ model: "gone-id", description }), true, description);
		}

		const next = mergePicker({ modelPicker: { options: [stale] } }, [
			{
		...
		assert.deepEqual(
			next.modelPicker.options.map((r) => r.model),
			["deepseek-flash[1m]"],
			"the de-curated row must be dropped, not preserved as foreign",
		);
	});
});
